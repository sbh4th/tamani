...
```{r att22, echo=FALSE, message=FALSE, results=FALSE}
d_agg %>% filter(time < 3) %>%
...
And we get:
```{r att23, message=FALSE, results=FALSE}
...
```{r att24, message=FALSE, results=FALSE}
...
```{r att32, message=FALSE, results=FALSE}
...
```{r gt33, message=FALSE, results=FALSE}
gt %>%
  kbl(digits=3, escape = FALSE) %>%
  kable_styling() %>%
...
```{r att33, message=FALSE, results=FALSE}
...
```{r att34, message=FALSE, results=FALSE}
...
```{r att42, message=FALSE, results=FALSE}
...
```{r att43, message=FALSE, results=FALSE}
...
And the resulting ATT(4,3) is:
```{r att44, message=FALSE, results=FALSE}
...
# Weighted ATTs
Instead of taking a weighted average estimate of the ATT with more-or-less arbitrary weights derived from the TWFE OLS approach. The CS DD approch considers several different ways of combinging the group-time ATTs we calculated above. 
